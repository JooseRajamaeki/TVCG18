...
__declspec( thread ) static dMessageFunction *error_function = 0;
__declspec( thread ) static dMessageFunction *debug_function = 0;
__declspec( thread ) static dMessageFunction *message_function = 0;
...
{
  va_list ap;
  dMessageFunction *errorFunction;

  va_start (ap,msg);

  errorFunction = dGetErrorHandler();

  if (errorFunction) errorFunction (num,msg,ap);
  else {
...
{
  va_list ap;
  dMessageFunction *debugFunction;
  va_start (ap,msg);
  
  debugFunction = dGetDebugHandler();

  if (debugFunction) debugFunction (num,msg,ap);
  else {
    char s[1000],title[100];
...
{
  va_list ap;
  dMessageFunction *messageFunction;

  va_start (ap,msg);
  
  messageFunction = dGetMessageHandler();

  if (messageFunction) messageFunction (num,msg,ap);
  else printMessage (num,"ODE Message",msg,ap);
}
